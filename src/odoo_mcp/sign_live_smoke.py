"""One-time idempotent live Odoo Sign document smoke test.

This module is intentionally conservative: it uploads a harmless PDF and creates
only a draft ``sign.send.request``. It never calls ``create_request`` and
therefore never sends email or initiates a signer ceremony.
"""

from __future__ import annotations

import sys
from typing import Any, Dict, List

TEST_FILENAME = "OdooSign MCP Test 2026-08-14.pdf"
TEST_SUBJECT = "OdooSign MCP Test 2026-08-14 - Draft Only"
TEST_PDF_BASE64 = ""

_STATUS: Dict[str, Any] = {
    "status": "not_run",
    "mode": "draft_only",
    "sent": False,
}


def get_document_smoke_status() -> Dict[str, Any]:
    """Return only non-secret smoke-test status for the public health route."""
    return dict(_STATUS)


def _created_ids(result: Any) -> List[int]:
    if isinstance(result, int):
        return [result]
    if isinstance(result, list):
        return [int(value) for value in result if isinstance(value, int) or str(value).isdigit()]
    return []


def _create(odoo: Any, model: str, values: Dict[str, Any]) -> Any:
    if getattr(odoo, "transport", "") == "json2":
        return odoo.execute_method(model, "create", vals_list=[values])
    return odoo.execute_method(model, "create", [values])


def _unlink(odoo: Any, model: str, record_id: int) -> None:
    if getattr(odoo, "transport", "") == "json2":
        odoo.execute_method(model, "unlink", ids=[record_id])
    else:
        odoo.execute_method(model, "unlink", [record_id])


def run_document_smoke(odoo: Any) -> None:
    """Upload the test PDF and create a draft Sign send wizard, never sending it."""
    global _STATUS
    created_attachment_id: int | None = None
    try:
        attachments = odoo.search_read(
            "ir.attachment",
            [["name", "=", TEST_FILENAME]],
            fields=["id", "name", "mimetype", "file_size"],
            limit=1,
        )
        if attachments:
            attachment_id = int(attachments[0]["id"])
            attachment_result = "already_present"
        else:
            attachment_result_raw = _create(
                odoo,
                "ir.attachment",
                {
                    "name": TEST_FILENAME,
                    "datas": TEST_PDF_BASE64,
                    "mimetype": "application/pdf",
                    "type": "binary",
                },
            )
            attachment_ids = _created_ids(attachment_result_raw)
            if not attachment_ids:
                raise RuntimeError("Odoo did not return an attachment ID")
            attachment_id = attachment_ids[0]
            created_attachment_id = attachment_id
            attachment_result = "created"

        existing_wizards = odoo.search_read(
            "sign.send.request",
            [["subject", "=", TEST_SUBJECT]],
            fields=["id", "subject", "attachment_ids"],
            limit=1,
        )
        if existing_wizards:
            wizard_id = int(existing_wizards[0]["id"])
            wizard_result = "already_present"
        else:
            wizard_result_raw = _create(
                odoo,
                "sign.send.request",
                {
                    "attachment_ids": [[6, 0, [attachment_id]]],
                    "subject": TEST_SUBJECT,
                    "reminder_enabled": False,
                    "reminder": 0,
                    "certificate_reference": False,
                    "set_sign_order": False,
                },
            )
            wizard_ids = _created_ids(wizard_result_raw)
            if not wizard_ids:
                raise RuntimeError("Odoo did not return a Sign draft wizard ID")
            wizard_id = wizard_ids[0]
            wizard_result = "created"

        verification = odoo.read_records(
            "sign.send.request",
            [wizard_id],
            ["subject", "attachment_ids"],
        )
        if not verification:
            raise RuntimeError("Sign draft wizard could not be read back")
        linked = [int(value) for value in verification[0].get("attachment_ids") or []]
        if attachment_id not in linked:
            raise RuntimeError("Sign draft wizard is not linked to the test PDF")

        _STATUS = {
            "status": "ok",
            "mode": "draft_only",
            "sent": False,
            "attachment": attachment_result,
            "draft": wizard_result,
        }
        print(
            "Odoo Sign document smoke: ok (draft created/verified; sent=false)",
            file=sys.stderr,
            flush=True,
        )
    except Exception as exc:
        if created_attachment_id is not None:
            try:
                _unlink(odoo, "ir.attachment", created_attachment_id)
            except Exception:
                pass
        _STATUS = {
            "status": "failed",
            "mode": "draft_only",
            "sent": False,
            "error_type": type(exc).__name__,
        }
        print(
            f"Odoo Sign document smoke: failed ({type(exc).__name__})",
            file=sys.stderr,
            flush=True,
        )
